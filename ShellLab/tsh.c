...
void do_bgfg(char **argv)
{
  ...
  if(*argv[1]=='%'){
    str = argv[1]+1;
    id = atoi(str);
    targetJob = getjobjid(jobs, id);
    if(targetJob == NULL){
      printf("%s: No such job\n", argv[1]);
      return;
    }
  }else if(0x30<=*argv[1] && *argv[1]<=0x39){
    str = argv[1];
    id = atoi(str);
    targetJob = getjobpid(jobs, id);
    if(targetJob == NULL){
  ...
  if(!strcmp(argv[0], "bg")){
    ...
    printf("[%d] (%d) %s", targetJob->jid, targetJob->pid, targetJob->cmdline);
  }

  if(!strcmp(argv[0], "fg")){
    targetJob -> state = FG;
    kill(-targetJob->pid, SIGCONT);
    waitfg(targetJob->pid);
  }
  return;
}
